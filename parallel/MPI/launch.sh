    ...
    do
        ...
        do
            ...
            do
                ...
                #launch qsub command
                qsub -N MPI_run${RUN}_pr${PROCESSES} -l select=${NUM_NODES}:ncpus=$(($CPUS_PER_NODE)):mem=4gb -l place=${PLACE} -o output/run_${RUN}/MPI_result_processes${PROCESSES}.log -e output/run_${RUN}/MPI_result_processes${PROCESSES}_err.log parallel.sh
                sleep 20 # ensure that the csv file has no race condition
            done
        done
    done
done