...
int main(){
    ...
    */
    ...
    float pr_in[NLAT][NLON], pr_out_global[NLAT][NLON], pr_out_local[NLAT][NLON];
    size_t start[] = {0, 0, 0}; // 3-dim array
    size_t count[] = {1, NLAT, NLON}; // 3-dim array 

    int time_per_process = ntime/world_size; // ATTENZIONE: cambiare il nome della variabile da ntime a nrecord per tutti, anche seriale
    int starttime = world_rank*time_per_process;
    int endtime = (world_rank + 1) * time_per_process;
    ...
    for (int time = starttime; time < endtime; time++){
        ...
        for(int lat = 0; lat < NLAT; lat++){ //MPI_Scatter applied here because we know pr_in, our vector
            for(int lon = 0; lon < NLON; lon++){
                pr_out_local[lat][lon] = pr_out_local[lat][lon] + pr_in[lat][lon];
            }
        }
    }
    printf("--- WR: %d pr_out_local[0][0] = %f\n", world_rank, pr_out_local[0][0]);

    MPI_Reduce(&pr_out_local, &pr_out_global, NLAT*NLON, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);

    MPI_Barrier(MPI_COMM_WORLD);

    if (world_rank == 0){
        printf("--- WR: %d pr_out_globa[0][0] = %f\n", world_rank, pr_out_global[0][0]);
        /* Get the average precipitations over the years for each point of the grid (average precipitations) */
        for(int lat = 0; lat < NLAT; lat++){
            for(int lon = 0; lon < NLON ; lon++){
                pr_out_global[lat][lon] /= ntime;
            }
        }
    }
    ...
    if ((retval = nc_put_vara_float(ncid, pr_varid, start, count, &pr_out_global[0][0]))) ERR(retval);
