        ...
        do
            ...
            do
                ...
                do
                    ...
                    #launch qsub command
                    qsub -Wblock=true -N opMP_MPI_run${RUN}_pr${PROCESSES}_th${THREADS} -l select=${NUM_NODES}:ncpus=$(($CPUS_PER_NODE)):mem=4gb -l place=${PLACE} -o output/run_${RUN}/openMP_MPI_result_processes${PROCESSES}_threads${THREADS}.log -e output/run_${RUN}/openMP_MPI_result_processes${PROCESSES}_threads${THREADS}_err.log parallel.sh
                done
            done
        done
    done
