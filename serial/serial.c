...
#define NLAT 192
#define NLON 288
#define NTIME 25*365 // 25 years x 365 days
...
int main(){
    ...
    if ((retval = nc_get_var_float(ncid, lat_varid, &lats[0]))) ERR(retval);
    ...
    */
    ...
    for(lat = 0; lat < NLAT; lat++)
        {
            for(lon = 0; lon < NLON ; lon++)
            {
                pr_out[lat][lon] = (pr_out[lat][lon])/NTIME;
            }
        }
    /* Close the file */
    ...
    if ((retval = nc_def_dim(ncid, TIME_NAME, NC_UNLIMITED, &time_dimid))) ERR(retval);
    ...
    if ((retval = nc_def_var(ncid, PR_NAME, NC_FLOAT, NDIMS, dimids, &pr_varid))) ERR(retval);

    /* Assign units attributes to the netCDF variables */
    if ((retval = nc_put_att_text(ncid, pr_varid, UNITS, strlen(PR_UNITS), PR_UNITS))) ERR(retval);
